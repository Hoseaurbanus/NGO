...
export default function PortalLayout() {
  ...
  const handleKeyDown = useCallback((e) => {
    ...
    if (e.key === 'Escape') {
      ...
      const focusable = sidebarRef.current?.querySelectorAll('a[href], button, [tabindex]:not([tabindex="-1"])')
      if (focusable?.length) {
  ...
  const sidebarContent = (
    <>
      ...
      <nav style={{ flex: 1, padding: '12px 8px', overflowY: 'auto' }} aria-label="Portal navigation">
        {portalLinks.map(link => (
          <Link key={link.path} to={link.path} onClick={!isDesktop ? closeSidebar : undefined} style={{
      ...
      <div style={{ padding: '16px', borderTop: '1px solid var(--glass-border)' }}>
        <Link to="/" onClick={!isDesktop ? closeSidebar : undefined} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 14px', borderRadius: 'var(--radius-md)', textDecoration: 'none', color: 'var(--text-secondary)', fontSize: '0.9375rem', marginBottom: '8px', minHeight: '44px' }}>
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--bg-primary)' }}>
      ...
      {/* Single Sidebar: fixed on both mobile and desktop, behavior via CSS */}
      <aside
        ref={sidebarRef}
        ...
        style={{
          ...
          boxShadow: isDesktop ? 'none' : '0 0 40px rgba(0, 0, 0, 0.5)',
      ...
      <main style={{
        ...
        <header style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginLeft: isDesktop ? '0' : 'auto' }}>
            <button style={{ background: 'none', border: 'none', color: 'var(--text-secondary)', fontSize: '1.1rem', cursor: 'pointer', position: 'relative', minWidth: '44px', minHeight: '44px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <i className="bi bi-bell"></i>
              <span style={{ position: 'absolute', top: '6px', right: '6px', width: '8px', height: '8px', borderRadius: '50%', background: 'var(--danger)' }}></span>
            </button>
          </div>
        </header>
        <div style={{ padding: isDesktop ? '24px' : '16px', overflowX: 'auto' }} className="portal-content">
          <Outlet />
        </div>
      </main>
